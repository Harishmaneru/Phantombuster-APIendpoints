 ...
 */
async function connectLinkedInAccount(userId, accountId, provider = 'LINKEDIN', name = null, metadata = {}) {
    try {
        const accountData = {
            user_id: userId,
        ...
        if (existingAccount) {
            ...
            console.log(`✅ Updated LinkedIn account for user ${userId}: ${accountId}`);
            return {
                success: true,
            ...
            console.log(`✅ Created new LinkedIn account for user ${userId}: ${accountId}`);
            return {
                success: true,
...
async function disconnectLinkedInAccount(userId, reason = 'User disconnected') {
    try {
        ...
        console.log(`❌ Disconnected LinkedIn account for user ${userId}: ${reason}`);
        return {
            success: true,
 ...
 * @returns {Promise<Object>} - Result of the operation
 */
async function refreshLinkedInAccount(userId, newAccountId, name = null, metadata = {}) {
    try {
        const result = await LinkedInAccounts.updateOne(
            { user_id: userId, provider: 'LINKEDIN' },
            {
        ...
        if (result.matchedCount === 0) {
            ...
            return await connectLinkedInAccount(userId, newAccountId, 'LINKEDIN', name, metadata);
        }

        console.log(`🔄 Refreshed LinkedIn account for user ${userId}: ${newAccountId}`);
        return {
            success: true,
...
async function handleAccountError(accountId, error) {
    try {
        ...
        console.log(`⚠️ Marked LinkedIn account ${accountId} as disconnected due to error: ${error}`);
        return {
            success: true,
