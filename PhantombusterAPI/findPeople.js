...
// const peopleFinderAgentId = '7350992633654709';
const peopleFinderAgentId = '1007026188768370';  
...
async function launchPhantombusterAgent(agentId, searchUrl, sessionCookie, agentArgs) {
    try {
...
router.post('/findpeople', async (req, res) => {
    const { searchUrl, sessionCookie } = req.body;
    
    if (!searchUrl || !sessionCookie) {
        return res.status(400).json({ error: 'Query URL and session cookie are required' });
    }

    try {
        console.log(`Processing request for Query URL: ${searchUrl}`);

        // Check for previously scraped data first
        const previousData = await findPreviousScrapedData(searchUrl);
        
        if (previousData) {
            console.log('Returning previously scraped data');
            
            // Check if there is an error message like "Page has crashed"
            if (previousData.resultObject.some(result => result.error === "Page has crashed")) {
                return res.status(400).json({ message: "You don't have a Sales Navigator account" });
            }
        ...
        console.log('Launching findpeople scraping agent');
        const containerId = await launchPhantombusterAgent(peopleFinderAgentId, searchUrl, sessionCookie);
        console.log(`findpeople scraping agent launched with container ID ${containerId}`);

        console.log('Waiting for agent to complete...');
        await waitForResults();

        console.log('Fetching agent results');
        const peopleData = await processScrapedData(containerId);
        ...
        if (peopleData.resultObject.some(result => result.error === "Page has crashed")) {
            return res.status(400).json({ message: "You don't have a Sales Navigator account" });
        }

        console.log('Saving scraped data to MongoDB');
        await saveToMongoDB('SalesNavigatorProfiles', {
            resultObject: peopleData.resultObject,
            containerOutput: peopleData.containerOutput,
