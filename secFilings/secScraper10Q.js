...
// router.get('/10-Qfilings/:ticker', async (req, res) => {
...
//     // let formTypes = req.query.formType || '10-Q';
//     let formTypes = '10-Q';
...
router.get('/10-Qfilings/:identifier', async (req, res) => {
  const startTime = Date.now();
  ...
  try {
    ...
    const result = await fetchFilings10Q(identifier, formTypes, startYear, endYear);
    const responseTime = Date.now() - startTime;
