...
const validateUserId = (req, res, next) => {
  ...
  if (!userId) {
    return res.status(400).json({
      ...
      details:
        "Please provide userId in request body, query parameters, or URL path",
    });
  }
...
async function getUserDomainsFromDatabase(userId, filters = {}) {
  try {
    ...
    console.log(
      `[Database] Retrieved ${domains.length} domains for user ${userId}`,
    );
    return domains;
  } catch (error) {
...
if (!NAMECHEAP_API_USER || !NAMECHEAP_API_KEY || !NAMECHEAP_CLIENT_IP) {
  ...
  throw new Error(
    "Server initialization failed: Missing Namecheap environment variables",
  );
}
...
if (!CPANEL_HOST || !CPANEL_USERNAME || !CPANEL_TOKEN || !WHM_TOKEN) {
  ...
  console.warn(
    "⚠️  Email functionality will be limited without WHM/cPanel configuration",
  );
}

// Log API configuration
console.log(
  `[Namecheap API] Mode: ${NAMECHEAP_SANDBOX === "true" ? "SANDBOX" : "PRODUCTION"}`,
);
...
// Mapping of ISO 2-letter country codes to international calling codes
...
/**
 * Formats a phone number strictly for Namecheap API: +CC.Number
 * CC = Calling Code (without leading +)
 * Number = Subscriber number (without spaces, dots, or leading zeros)
 *
 * @param {string} phone - Raw phone number
 * @param {string} countryIso - ISO 2-letter country code (e.g. "US", "IN")
 * @returns {string} Formatted number or raw if failing validation
 */
function formatPhoneForNamecheap(phone, countryIso) {
  if (!phone) return phone;
  if (!countryIso) return phone;

  // Clean the phone number of ALL non-digits
  let digits = phone.replace(/[^\d]/g, "");
  const iso = countryIso.toUpperCase();
  const callingCode = COUNTRY_CALLING_CODES[iso];

  if (!callingCode) {
    console.warn(
      `[Phone Format] ⚠️ Unknown country ISO: ${countryIso}. Falling back to heuristic.`,
    );
  ...
  // Handle case where phone might already include the calling code
  if (digits.startsWith(callingCode)) {
    // If it starts with calling code, ensure we don't double it
    // But be careful: some numbers start with same digits as CC
    // Namecheap wants +CC.Number
...
function getNameserverConfig(
  ...
  if (useNamecheapDNS) {
    ...
    customNameservers &&
    Array.isArray(customNameservers) &&
    customNameservers.length >= 2
  ) {
    ...
    if (validNameservers.length >= 2) {
      ...
      throw new Error(
        "Invalid custom nameservers provided. Please provide at least 2 valid nameserver addresses.",
      );
    }
  } else {
...
async function setDomainNameservers(domain, nameserverConfig) {
  ...
  if (nameserverConfig.isNamecheapDNS) {
    ...
    if (
      response?.ApiResponse?.CommandResponse?.DomainDNSSetDefaultResult?.$
        .Updated !== "true"
    ) {
    ...
    if (
      response?.ApiResponse?.CommandResponse?.DomainDNSSetCustomResult?.$
        .Updated !== "true"
    ) {
  ...
  try {
    ...
    if (typeof response.data === "string") {
      if (
        ...
        throw new Error(
          "HTTP method not allowed. The API endpoint requires a different HTTP method.",
        );
      }
      if (!response.data.trim().startsWith("<?xml")) {
        console.error(
          "[Namecheap API] Unexpected response format:",
          response.data,
        );
    ...
    if (parsed.ApiResponse.$.Status !== "OK") {
      throw new Error(
        `API returned non-OK status: ${parsed.ApiResponse.$.Status}`,
      );
    }
    ...
    if (retryCount < maxRetries) {
      ...
      if (shouldRetry) {
        console.log(
          `[Namecheap API] Retrying request (attempt ${retryCount + 1}/${maxRetries})...`,
        );
        ...
        if (err.response?.status === 405) {
          ...
          try {
            ...
            console.error(
              "[Namecheap API] POST retry failed:",
              postError.message,
            );
          }
        }
  ...
  try {
    ...
    categories.forEach((category) => {
      ...
      const additionalCost = parseFloat(
        oneYearPrice.$.YourAdditonalCost || "0",
      );
      ...
      pricing.detailed[categoryName] = {
        ...
        regularPrice: parseFloat(
          oneYearPrice.$.RegularPrice || oneYearPrice.$.Price,
        ),
        regularPriceType: oneYearPrice.$.RegularPriceType,
        regularAdditionalCost: parseFloat(
          oneYearPrice.$.RegularAdditionalCost || "0",
        ),
...
function extractPriceForDuration(xml, years) {
  try {
    ...
    categories.forEach((category) => {
      ...
      const requestedYearPrice = prices.find(
        (p) => p.$.Duration === years.toString(),
      );
      ...
      if (requestedYearPrice) {
        ...
        const additionalCost = parseFloat(
          requestedYearPrice.$.YourAdditonalCost || "0",
        );
        ...
        switch (category.$.Name.toLowerCase()) {
          case "register":
            ...
            pricing.perYearCost =
              (totalPrice + pricing.icannFee) / parseInt(years);
            break;
          case "renew":
      ...
      if (oneYearPrice && category.$.Name.toLowerCase() === "register") {
        const oneYearTotal =
          parseFloat(oneYearPrice.$.YourPrice) +
          parseFloat(oneYearPrice.$.YourAdditonalCost || "0") +
          0.18;
        ...
        if (pricing.totalCost) {
          pricing.savings = {
            ...
            percentage: (
              ((multiYearEquivalent - pricing.totalCost) /
                multiYearEquivalent) *
              100
            ).toFixed(2),
            comparedToYearly: {
    ...
    console.error(
      "[Domain API] Error extracting multi-year price:",
      err.message,
    );
    return null;
  }
...
async function generateProductionSuggestions(
  keyword,
  originalTld,
  isPrimaryAvailable,
  ...
  console.log(
    `[Suggestion Engine] Generating suggestions for "${keyword}.${originalTld}" (Available: ${isPrimaryAvailable})`,
  );
  ...
  try {
    ...
    const tldPromises = POPULAR_TLDS.filter(
      (tld) => tld !== originalTld.toLowerCase(),
    )
      ...
      .map(async (tld) => {
        ...
        try {
          ...
          const domainResult =
            check.ApiResponse.CommandResponse.DomainCheckResult;
          ...
          const price = domainResult.$.Price
            ? parseFloat(domainResult.$.Price)
            : null;
          const eapFee = domainResult.$.EapFee
            ? parseFloat(domainResult.$.EapFee)
            : 0;
          ...
          if (isValid) {
            ...
            try {
              ...
              console.warn(
                `[Suggestion Engine] Pricing failed for ${variant}:`,
            ...
            try {
              ...
              console.warn(
                `[Suggestion Engine] Privacy info failed for ${variant}:`,
          ...
          console.warn(
            `[Suggestion Engine] Error checking ${variant}:`,
            error.message,
          );
        }
        return null;
    ...
    if (!isPrimaryAvailable || results.tldVariations.length < 4) {
      console.log(
        `[Suggestion Engine] Generating keyword variations for "${keyword}"`,
      );
      ...
      VARIATION_PATTERNS.forEach((pattern) => {
        ...
        const testTlds = [originalTld.toLowerCase(), "com"].filter(
          (tld, index, arr) => arr.indexOf(tld) === index,
        );
              ...
              namecheapRequest("namecheap.domains.check", {
                ...
                .then(async (check) => {
                  ...
                  const isValid =
                    isAvailable &&
                    (!isPremium ||
                      (isPremium && price && acceptPremiumPricing)) &&
                    domainResult.$.ErrorNo === "0";
                  ...
                  if (isValid) {
                    ...
                    try {
                      ...
                      console.warn(
                        `[Suggestion Engine] Keyword variation pricing failed for ${domain}:`,
                        pricingError.message,
                      );
                      pricing = {
                        register: null,
                    ...
                    try {
                      ...
                      console.warn(
                        `[Suggestion Engine] Keyword variation privacy info failed for ${domain}:`,
                        privacyError.message,
                      );
                      privacyInfo = {
                        supported: null,
                    ...
                    return {
                      ...
                      privacyProtection: {
                        ...
                        cost: privacyInfo?.pricing?.cost || null,
                        note:
                          privacyInfo?.pricing?.note || "Unable to determine",
                        dataSource: privacyInfo?.dataSource || "error_fallback",
                      },
                      variation: variant,
                ...
                .catch((error) => {
                  console.warn(
                    `[Suggestion Engine] Error checking keyword variation ${domain}:`,
                    error.message,
                  );
                  return null;
                }),
    ...
    console.log(
      `[Suggestion Engine] Generated ${results.tldVariations.length} TLD variations and ${results.keywordVariations.length} keyword variations`,
    );

    return results;
  } catch (error) {
    console.error(
      "[Suggestion Engine] Error generating suggestions:",
      error.message,
    );
...
async function getPricingForTLD(tld) {
  ...
  try {
    console.log(
      `[Pricing API] 🌐 Making Namecheap pricing API call for .${tld.toUpperCase()}`,
    );
    ...
    console.log(
      `[Pricing API] 💾 Cached pricing for .${tld} (expires in 1 hour)`,
    );
    return pricing;
  } catch (error) {
    ...
    console.log(
      `[Pricing API] 🔄 Using fallback pricing for .${tld}:`,
      fallbackPricing,
    );
...
function generateDomainGroups(suggestions, originalTld) {
  ...
  const premiumDomains = [
    ...suggestions.tldVariations,
    ...suggestions.keywordVariations,
...
async function configureEmailDns(domain) {
  ...
  try {
    ...
    console.error(
      "[Email DNS] Failed to fetch DKIM key from WHM:",
      err.message,
    );
  ...
  const records = [
    ...
    },
    {
      HostName: "@",
      RecordType: "TXT",
      Address: `v=spf1 a mx ip4:${ip} ~all`,
      TTL: "1800",
    },
    {
  ...
  try {
    ...
    const successFlag =
      response?.ApiResponse?.CommandResponse?.DomainDNSSetHostsResult?.$
        ?.IsSuccess === "true";
...
router.get("/namecheap/wallet/balance", async (req, res) => {
  try {
    ...
    const balanceResult =
      response.ApiResponse.CommandResponse.UserGetBalancesResult;
    ...
    const availableBalance =
      parseFloat(balanceResult.$.AvailableBalance) || accountBalance;
    const fundsOnHold =
      parseFloat(balanceResult.$.FundsRequiredForAutoRenew) || 0;
    ...
    res.json({
      ...
      data: {
        ...
        recommendations: {
          ...
          suggestedTopUp:
            accountBalance < 50
              ? Math.ceil((100 - accountBalance) / 10) * 10
              : 0,
          autoRenewBuffer:
            fundsOnHold > 0
              ? "Funds reserved for auto-renewal"
              : "No auto-renewal reservations",
        },
      },
      timestamp: new Date().toISOString(),
...
router.get("/namecheap/wallet/account-info", async (req, res) => {
  try {
    ...
    const balanceResponse = await namecheapRequest(
      "namecheap.users.getBalances",
    );
    const balanceResult =
      balanceResponse.ApiResponse.CommandResponse.UserGetBalancesResult;

    // Try to get pricing information for common TLDs to show spending estimates
    let pricingInfo = {};
    try {
      const pricingResponse = await namecheapRequest(
      ...
      const productType =
        pricingResponse.ApiResponse.CommandResponse.UserGetPricingResult
          .ProductType;
      const categories = Array.isArray(productType.ProductCategory)
        ? productType.ProductCategory
        : [productType.ProductCategory];
      ...
      console.warn(
        "[Account API] Could not fetch pricing info:",
        pricingError.message,
      );
    }
    ...
    const estimatedDomains = {
      com: pricingInfo.com
        ? Math.floor(accountBalance / pricingInfo.com.register)
        : 0,
      net: pricingInfo.net
        ? Math.floor(accountBalance / pricingInfo.net.register)
        : 0,
      org: pricingInfo.org
        ? Math.floor(accountBalance / pricingInfo.org.register)
        : 0,
    };
    ...
    res.json({
      ...
      data: {
        ...
        capabilities: {
          ...
          minimumForRegistration:
            Math.min(
              ...Object.values(pricingInfo)
                .map((p) => p.register)
                .filter(Boolean),
            ) || 10,
        },
...
router.get("/namecheap/domain/check/:domain", async (req, res) => {
  ...
  try {
    ...
    console.log(
      `[Domain API] Step 1: Primary availability check for ${domain}`,
    );
    ...
    try {
      ...
      icannFee = domainResult.$.IcannFee
        ? parseFloat(domainResult.$.IcannFee)
        : 0.18;
    } catch (primaryError) {
      console.error(
        `[Domain API] Primary check failed for ${domain}:`,
    ...
    try {
      ...
      console.warn(
        `[Domain API] Pricing fetch failed for ${domain}:`,
        pricingError.message,
      );
      primaryPricing = {
        register: null,
    ...
    try {
      console.log(
        `[Domain API] Step 2.5: Getting privacy protection info for .${originalTld}`,
      );
      privacyInfo = await getPrivacyProtectionInfo(originalTld);
    } catch (privacyError) {
      console.warn(
        `[Domain API] Privacy info fetch failed for ${domain}:`,
        privacyError.message,
      );
      privacyInfo = {
        supported: null,
    ...
    try {
      ...
      const suggestionsPromise = generateProductionSuggestions(
        keyword,
        originalTld,
        isAvailable,
        false,
      ...
      suggestions = await Promise.race([
        ...
        new Promise((resolve) =>
          setTimeout(() => {
            console.log(
              "[Domain API] Suggestions generation timed out, using empty results",
            );
            resolve({
              tldVariations: [],
              keywordVariations: [],
        ...
        ), // Reduced to 8 second timeout
      ]);
    } catch (suggestionsError) {
      console.warn(
        `[Domain API] Suggestions generation failed for ${domain}:`,
    ...
    try {
      ...
      console.warn(
        `[Domain API] Groups generation failed for ${domain}:`,
        groupsError.message,
      );
      groups = [];
    }
    ...
    const totalSuggestions =
      suggestions.tldVariations.length + suggestions.keywordVariations.length;
    const premiumCount = [
      ...suggestions.tldVariations,
      ...suggestions.keywordVariations,
    ...
    const response = {
      ...
      data: {
        ...
        summary: {
          ...
          bestAlternative:
            totalSuggestions > 0
              ? suggestions.tldVariations?.[0] ||
                suggestions.keywordVariations?.[0]
              : null,
        },
    ...
    console.log(
      `[Domain API] ✅ Check completed for ${domain} in ${Date.now() - startTime}ms`,
    );
    console.log(
      `[Domain API] Results: Available=${isAvailable}, Premium=${isPremium}, Suggestions=${totalSuggestions}`,
    );
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      console.log(
        `[Bulk Pricing API] Checking ${domains.length} domains for ${years} year(s)`,
      );

      // Step 1: Bulk availability check with timeout
      const domainList = domains.join(",");
      console.log(
        `[Bulk Pricing API] Making bulk availability check for: ${domainList}`,
      );
      ...
      const checkResults =
        bulkCheck.ApiResponse.CommandResponse.DomainCheckResult;
      const resultsArray = Array.isArray(checkResults)
        ? checkResults
        : [checkResults];
      ...
      console.log(
        `[Bulk Pricing API] Getting pricing for ${uniqueTlds.length} unique TLDs: ${uniqueTlds.join(", ")}`,
      );
      ...
      const tldPricingPromises = uniqueTlds.map(async (tld) => {
        try {
          if (yearsInt === 1) {
            ...
            const priceXmlPromise = namecheapRequest(
              "namecheap.users.getPricing",
              {
                ProductType: "DOMAIN",
                ProductCategory: "REGISTER",
            ...
            try {
              ...
              console.warn(
                `[Bulk Pricing API] Failed to extract pricing for .${tld}:`,
                extractionError.message,
              );
              throw new Error(
                `Pricing extraction failed: ${extractionError.message}`,
              );
            }
          ...
          console.warn(
            `[Bulk Pricing API] Failed to get pricing for .${tld}:`,
            error.message,
          );
          return {
            tld,
      ...
      const results = resultsArray.map((domainResult) => {
        ...
        const price = domainResult.$.Price
          ? parseFloat(domainResult.$.Price)
          : null;
        const icannFee = domainResult.$.IcannFee
          ? parseFloat(domainResult.$.IcannFee)
          : 0.18;

        const tld = domain.split(".").pop().toUpperCase();
        const tldPricing = tldPricingMap[tld] || {
          register: null,
          renew: null,
          transfer: null,
        };
      ...
      console.log(
        `[Bulk Pricing API] ✅ Processed ${domains.length} domains in ${Date.now() - startTime}ms`,
      );
...
router.get(
  "/namecheap/domain/:domain/pricing",
  apiLimiter,
  asyncHandler(async (req, res) => {
    ...
    console.log(
      `[Domain Pricing API] 💰 Getting ${years}-year pricing for domain: ${domain} (query param version)`,
    );
    ...
    try {
      ...
      console.log(
        `[Domain Pricing API] Step 1: Checking availability for ${domain}`,
      );
      ...
      const domainResult =
        checkResult.ApiResponse.CommandResponse.DomainCheckResult;
      ...
      const tld = domain.split(".").pop().toUpperCase();
      console.log(
        `[Domain Pricing API] Step 2: Fetching ${years}-year pricing for TLD ${tld}`,
      );
      ...
      if (!priceXml) {
        ...
        console.log(
          `[Domain Pricing API] Using cached pricing data for .${tld} (${years} years)`,
        );
      }
      ...
      try {
        ...
        console.error(
          `[Domain Pricing API] Error extracting pricing for ${domain}:`,
          extractionError.message,
        );
      ...
      if (!pricing || typeof pricing !== "object") {
        console.error(
          `[Domain Pricing API] Invalid pricing data for ${domain}:`,
          pricing,
        );
        return res.status(500).json({
          success: false,
  ...
  try {
    ...
    if (error.message.includes("Invalid request IP")) {
      const errorMatch = error.message.match(
        /Invalid request IP: (\d+\.\d+\.\d+\.\d+)/,
      );
      const ipAddress = errorMatch
        ? errorMatch[1]
      ...
      return res.status(403).json({
        ...
        details: {
          message:
            "Your server IP is not whitelisted in Namecheap API settings",
          steps: [
    ...
    if (
      error.message.includes("premium domain") ||
      error.message.includes("Premium price")
    ) {
      return res.status(400).json({
        success: false,
        ...
        details: {
          message:
            "This is a premium domain that requires additional pricing information",
          domain,
          isPremium: true,
          steps: [
...
class RateLimitedQueue {
  constructor(maxCallsPerMinute = 18) {
...
router.get("/namecheap/domains/list", validateUserId, async (req, res) => {
  ...
  try {
    console.log(
      `[Domain API] 📋 Fetching domains for user: ${userId}, live: ${includeLiveStatus}`,
    );
    ...
    const fetchDomainDetails = async (dbDomain) => {
      ...
      try {
        ...
        const namecheapResult =
          domainInfo.ApiResponse.CommandResponse.DomainGetInfoResult;
        ...
        domainObj.liveStatus = {
          ...
          nameservers: Array.isArray(namecheapResult.DnsDetails.Nameserver)
            ...
            : [namecheapResult.DnsDetails.Nameserver],
          isUsingNamecheapDNS:
            namecheapResult.DnsDetails.$.IsUsingOurDNS === "true",
        };
        ...
        if (namecheapResult.DnsDetails.$.IsUsingOurDNS === "true") {
          try {
            ...
            const hosts =
              dnsHosts.ApiResponse.CommandResponse?.DomainDNSGetHostsResult
                ?.host;
            ...
            const liveRedirects = hostsList
              .filter(
                (host) =>
                  host.$ && ["URL301", "URL302", "FRAME"].includes(host.$.Type),
              )
              .map((host) => ({
                type: host.$.Type,
                address: host.$.Address,
            ...
            console.warn(
              `[Domain API] DNS hosts fetch failed for ${dbDomain.domain}:`,
        ...
        if (
          domainObj.domainStatus.autoRenew !==
          (namecheapResult.$.AutoRenew === "true")
        ) {
          updateData["domainStatus.autoRenew"] =
            namecheapResult.$.AutoRenew === "true";
          needsUpdate = true;
        }

        if (
          domainObj.domainStatus.isLocked !==
          (namecheapResult.$.IsLocked === "true")
        ) {
          updateData["domainStatus.isLocked"] =
            namecheapResult.$.IsLocked === "true";
          needsUpdate = true;
        }
        ...
        if (needsUpdate) {
          ...
          updateDomainInDatabase(userId, dbDomain.domain, updateData)
            .then(() =>
              console.log(
                `[Domain API] Updated live status for ${dbDomain.domain}`,
              ),
            )
            .catch((err) =>
              console.warn(
                `[Domain API] Background update failed for ${dbDomain.domain}:`,
                err.message,
              ),
            );
        }
        ...
        console.error(
          `[Domain API] Live status fetch failed for ${dbDomain.domain}:`,
          namecheapError.message,
        );
    ...
    for (const dbDomain of userDomains) {
      try {
        const domainDetails = await Promise.race([
          ...
          new Promise((_, reject) =>
            setTimeout(
              () => reject(new Error("Domain fetch timeout")),
              timeout,
            ),
          ),
        ]);
        ...
        // Progress logging
        console.log(
          `[Domain API] Processed ${processedCount}/${userDomains.length} domains for user ${userId}`,
        );
      } catch (error) {
        console.error(
          `[Domain API] Failed to process domain ${dbDomain.domain}:`,
          error.message,
        );
      ...
      data: {
        ...
        sandboxWarning:
          NAMECHEAP_SANDBOX === "true"
            ? "Running in sandbox mode - Live status may not be accurate"
            : null,
      },
    });
  } catch (err) {
...
router.post("/namecheap/domain/redirect", validateUserId, async (req, res) => {
  ...
  if (!domain || !destinationUrl) {
    return res.status(400).json({
      success: false,
      error:
        "Missing required fields: userId, domain and destinationUrl are required",
    });
  }
  ...
  try {
    ...
    console.log(
      `[Redirect API] Setting up redirect for ${domain} (User: ${userId})`,
    );
    ...
    const nameserverList = Array.isArray(nameservers)
      ? nameservers
      : [nameservers];
    ...
    const existingRecords = await namecheapRequest(
    ...
    const currentHosts =
      existingRecords.ApiResponse.CommandResponse.DomainDNSGetHostsResult
        .host || [];
    const hostsArray = Array.isArray(currentHosts)
      ? currentHosts
      : [currentHosts];

    const preservedRecords = hostsArray
      .filter((record) => {
        ...
        if (
          hostName === "@" &&
          ["A", "CNAME", "URL301", "URL302", "FRAME"].includes(recordType)
        ) {
          return false;
        }
        return true;
    ...
    console.log(
      `[Redirect API] Updating DNS with ${allRecords.length} records`,
    );
    ...
    allRecords.forEach((record, index) => {
      ...
      if (record.Description)
        hostParams[`Description${i}`] = record.Description;
    });

    const updateResponse = await namecheapRequest(
    ...
    if (
      error.message.includes("Domain name not found") ||
...
router.put("/namecheap/domain/:domain/auto-renew", async (req, res) => {
  ...
  try {
    ...
    if (currentAutoRenew !== autoRenew) {
      ...
      if (
        response?.ApiResponse?.CommandResponse?.DomainSetAutoRenewResult?.$
          .IsSuccess !== "true"
      ) {
...
router.get(
  "/namecheap/domain/:domain/emails",
  validateUserId,
  asyncHandler(async (req, res) => {
    const { domain } = req.params;
    const userId = req.userId;
    ...
    try {
      ...
      try {
        ...
        if (cpanelResponse.status) {
          ...
          cpanelError =
            cpanelResponse.errors?.[0] || "Failed to list email accounts";
        }
      } catch (error) {
        cpanelError = error.message;
        console.warn(
          `[Email API] cPanel error for ${domain}, using database data:`,
          error.message,
        );
      }
      ...
      liveEmailAccounts.forEach((liveAccount) => {
        combinedAccounts.push({
          ...liveAccount,
          inDatabase: databaseAccounts.some(
            (dbAccount) => dbAccount.email === liveAccount.email,
          ),
      ...
      databaseAccounts.forEach((dbAccount) => {
        const foundInLive = liveEmailAccounts.find(
          (liveAccount) => liveAccount.email === dbAccount.email,
        );
        if (!foundInLive) {
          combinedAccounts.push({
      ...
      console.error(
        `[Email API] Error listing emails for ${domain} (User: ${userId}):`,
        {
          error: error.message,
          domain,
          stack: error.stack,
...
router.get(
  "/namecheap/domain/transfer/status/:domain",
  validateUserId,
  asyncHandler(async (req, res) => {
    const { domain } = req.params;
    const userId = req.userId;
    ...
    try {
      ...
      res.json({
        ...
        domain,
        status:
          response.ApiResponse.CommandResponse.DomainTransferGetStatusResult,
        timestamp: new Date().toISOString(),
      });
    } catch (error) {
      console.error(
        `[Transfer Status API] Error checking status for ${domain}:`,
        error.message,
      );
      res.status(500).json({
        success: false,
...
router.get(
  "/namecheap/user/:userId/contact-info",
  validateUserId,
  asyncHandler(async (req, res) => {
    const userId = req.userId;
    ...
    try {
      console.log(
        `[Contact API] 📋 Fetching latest contact information for user: ${userId}`,
      );
      ...
      const domainsWithSameContact = userDomains
        .filter((domain) => {
          return (
            domain.contactInfo &&
      ...
      console.error(
        `[Contact API] ❌ Error fetching latest contact info for user ${userId}:`,
        {
          error: error.message,
          stack: error.stack,
        },
...
async function getPrivacyProtectionInfo(tld) {
  ...
  if (privacyCache.has(cacheKey)) {
    ...
    if (Date.now() - cached.timestamp < 3600000) {
      // 1 hour
      console.log(
        `[Privacy API] ✅ Using cached privacy info for .${tld}:`,
        cached.info,
      );
      return cached.info;
    }
  ...
  try {
    console.log(
      `[Privacy API] 🔍 Checking privacy protection support for .${tld} via Namecheap API`,
    );
    ...
    try {
      ...
      if (pricingResponse?.ApiResponse?.CommandResponse?.UserGetPricingResult) {
        ...
        console.log(
          `[Privacy API] ✅ Privacy protection confirmed via pricing API for .${tld}`,
        );
      }
    } catch (pricingError) {
      // If pricing API fails, it might mean privacy protection is not supported
      console.log(
        `[Privacy API] ⚠️ Privacy pricing API failed for .${tld}:`,
        pricingError.message,
      );
    }
    ...
    if (privacySupported === null) {
      console.log(
        `[Privacy API] 🔍 Checking .${tld} against known restrictions`,
      );
      ...
      console.log(
        `[Privacy API] 📋 Based on known restrictions, .${tld} privacy support: ${privacySupported}`,
      );
    }
    ...
    const privacyInfo = {
      ...
      pricing: {
        ...
        currency: "USD",
        period: "yearly",
        note: privacySupported
          ? "Free with domain registration"
          : "Not supported for this TLD",
      },
      ...
      dataSource:
        privacySupported === true && privacyCost === 0
          ? "namecheap_api"
          : "known_restrictions",
    };
    ...
    console.error(
      `[Privacy API] ❌ Error getting privacy info for .${tld}:`,
      error.message,
    );
...
async function revalidateDomainAvailability(
  domain,
  acceptPremiumPricing = false,
) {
  console.log(
    `[Domain Validation] Revalidating availability for domain: ${domain}`,
  );
  ...
  try {
    ...
    console.error(
      `[Domain Validation] Error revalidating domain ${domain}:`,
      error,
    );
    throw error;
  }
...
async function registerDomainWithNamecheap(registrationData) {
  ...
  try {
    ...
    if (
      ...
      throw new Error(
        "Missing required registration fields: domain, firstName, lastName, email, phone, address1, city, stateProvince, country, postalCode are required",
      );
    }
    ...
    try {
      nameserverConfig = getNameserverConfig(
        customNameservers,
        useNamecheapDNS,
      );
    } catch (error) {
    ...
    if (!isValidDomain(domain)) {
      throw new Error(
        "Invalid domain format - Domain must be a valid format and support modern TLDs",
      );
    }
    ...
    if (existingDomain) {
      throw new Error(
        `Domain already registered for this user. Registration date: ${existingDomain.registrationData.registrationDate}, Expiration: ${existingDomain.registrationData.expirationDate}`,
      );
    }

    // Revalidate domain availability before proceeding with registration
    const validationResult = await revalidateDomainAvailability(
      domain,
      acceptPremiumPricing,
    );
    ...
    console.log(
      `[Domain Registration] Registering domain: ${domain} (User: ${userId}), Premium: ${isPremium}`,
    );
    const registrationParams = {
      ...
      RegistrantPhone: formatPhoneForNamecheap(phone, country),
      ...
      TechPhone: formatPhoneForNamecheap(phone, country),
      ...
      AdminPhone: formatPhoneForNamecheap(phone, country),
      ...
      AuxBillingPhone: formatPhoneForNamecheap(phone, country),
    ...
    if (isPremium) {
      console.log(
        `[Domain Registration] Adding premium pricing - Price: ${price}, EAP Fee: ${domainResult.$.EapFee || 0}`,
      );
    ...
    const registrationResult = await namecheapRequest(
    ...
    console.log(
      `[Domain Registration] Checking default nameservers assigned by Namecheap for ${domain}`,
    );
    const domainInfo = await namecheapRequest("namecheap.domains.getInfo", {
      DomainName: domain,
    });

    const registeredDomainResult =
      domainInfo.ApiResponse.CommandResponse.DomainGetInfoResult;
    const defaultNameservers = Array.isArray(
      registeredDomainResult.DnsDetails.Nameserver,
    )
      ? registeredDomainResult.DnsDetails.Nameserver
      : [registeredDomainResult.DnsDetails.Nameserver];
    const isUsingNamecheapDNS =
      registeredDomainResult.DnsDetails.$.IsUsingOurDNS === "true";
    ...
    if (!nameserverConfig.useDefaults) {
      console.log(
        `[Domain Registration] Setting custom nameservers for ${domain}:`,
        nameserverConfig,
      );
      ...
      console.log(
        `[Domain Registration] Using Namecheap default nameservers for ${domain}:`,
        defaultNameservers,
      );
      nameserverResult = {
        success: true,
    ...
    const domainData = {
      ...
      registrationData: {
        domainId:
          registrationResult.ApiResponse.CommandResponse.DomainCreateResult.$
            .DomainID,
        orderId:
          registrationResult.ApiResponse.CommandResponse.DomainCreateResult.$
            .OrderID,
        transactionId:
          registrationResult.ApiResponse.CommandResponse.DomainCreateResult.$
            .TransactionID,
        chargedAmount: parseFloat(
          registrationResult.ApiResponse.CommandResponse.DomainCreateResult.$
            .ChargedAmount,
        ),
        registrationDate: new Date(),
        expirationDate: new Date(
          Date.now() + parseInt(years) * 365 * 24 * 60 * 60 * 1000,
        ),
        years: parseInt(years),
      },
      contactInfo: {
    ...
    if (stripePaymentInfo) {
      ...
      console.log(
        `[Domain Registration] Added Stripe payment info for ${domain}:`,
        {
          sessionId: stripePaymentInfo.sessionId,
          amountPaid: stripePaymentInfo.amountPaid,
          paymentStatus: stripePaymentInfo.paymentStatus,
    ...
    const result = {
      ...
      nextSteps: {
        ...
        emailSetup: {
          status: "pending_dns",
          instructions:
            "Please wait for DNS propagation before creating email accounts",
      ...
      sandboxWarning:
        process.env.NAMECHEAP_SANDBOX === "true"
          ? "Running in sandbox mode - Domain registration is simulated"
          : null,
    };
    ...
    console.log(
      `[Domain Registration] ✅ Successfully registered ${domain} for user ${userId}`,
    );
    return result;
  } catch (error) {
    ...
    console.error(
      `[Domain Registration] ❌ Failed to register ${domain} for user ${userId}:`,
      error.message,
    );
    throw error;
  }
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      const contactInfo =
        result.ApiResponse.CommandResponse.DomainContactsResult;
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      const result = await retryWithBackoff(async () => {
        return await namecheapBreaker.fire(
          "namecheap.domains.transfer.getStatus",
          {
            DomainName: domain,
          },
      ...
      const statusResult = {
        ...
        domain,
        status:
          result.ApiResponse.CommandResponse.TransferGetStatusResult.Status,
        transferable:
          result.ApiResponse.CommandResponse.TransferGetStatusResult
            .Transferable === "true",
        authCodeRequired:
          result.ApiResponse.CommandResponse.TransferGetStatusResult
...
router.get(
  "/namecheap/domain/:domain/nameservers",
  validateUserId,
  async (req, res) => {
    const { domain } = req.params;
    const userId = req.userId;
    ...
    try {
      ...
      console.log(
        `[Nameserver API] Getting nameserver configuration for ${domain} (User: ${userId})`,
      );

      // 2. Get current nameserver info from Namecheap
      const domainInfo = await namecheapRequest("namecheap.domains.getInfo", {
        DomainName: domain,
      });
      ...
      const domainResult =
        domainInfo.ApiResponse.CommandResponse.DomainGetInfoResult;
      const currentNameservers = Array.isArray(
        domainResult.DnsDetails.Nameserver,
      )
        ? domainResult.DnsDetails.Nameserver
        : [domainResult.DnsDetails.Nameserver];
      const isUsingNamecheapDNS =
        domainResult.DnsDetails.$.IsUsingOurDNS === "true";
      ...
      if (isUsingNamecheapDNS) {
        try {
          ...
          const dnsHosts = await namecheapRequest(
            "namecheap.domains.dns.getHosts",
            {
              SLD: sld,
              TLD: tld,
          ...
          const hosts =
            dnsHosts.ApiResponse.CommandResponse?.DomainDNSGetHostsResult?.host;
          if (hosts) {
            const hostsArray = Array.isArray(hosts) ? hosts : [hosts];
            dnsRecords = hostsArray.map((host) => ({
              name: host.$.Name,
              type: host.$.Type,
              address: host.$.Address,
              ttl: host.$.TTL,
          ...
          console.warn(
            `[Nameserver API] Could not fetch DNS records for ${domain}:`,
            error.message,
          );
        }
      }
      ...
      const response = {
        ...
        data: {
          ...
          currentConfiguration: {
            ...
            type: isUsingNamecheapDNS ? "namecheap_dns" : "custom",
      ...
      console.error(
        `[Nameserver API] ❌ Error getting nameserver config for ${domain} (User: ${userId}):`,
        {
          error: error.message,
          stack: error.stack,
        },
...
router.post(
  "/namecheap/domain/nameserver/create",
  validateUserId,
  async (req, res) => {
    const { domain, nameserver, ipAddress } = req.body;
    ...
    try {
      ...
      console.log(
        `[Nameserver API] Creating nameserver ${nameserver} for ${domain} (User: ${userId})`,
      );
      ...
      const createResult = await namecheapRequest(
        ...
        Result: [
          {
            ...
            IP: ipAddress || "",
            IsSuccess:
              createResult.ApiResponse.CommandResponse.DomainNSCreateResult.$
                .IsSuccess === "true",
            ErrorNo: "",
            Description: "",
            Message: null,
...
async function whmRequest(apiFunction, params) {
  try {
    ...
    if (!whmHost || !whmToken) {
      throw new Error(
        "WHM_HOST and WHM_TOKEN environment variables are required",
      );
    }
...
router.post("/whm/domain/get-ip", validateUserId, async (req, res) => {
  ...
  try {
    console.log(
      `[WHM Domain IP Lookup API] Getting IP for ${domain} (User: ${userId})`,
    );
    ...
    if (
      ...
      resolveResult.metadata.result === 1
    ) {
      return res.json({
        success: true,
    ...
    console.error(
      "[WHM Domain IP Lookup API] Error getting domain IP:",
      error.message,
    );
...
router.post(
  "/namecheap/domain/nameserver/set-custom",
  validateUserId,
  async (req, res) => {
    ...
    const nameserverArray = Array.isArray(nameservers)
      ? nameservers
      : nameservers.split(",");
    ...
    try {
      ...
      console.log(
        `[Set Custom Nameservers API] Setting custom nameservers for ${domain} (User: ${userId}):`,
        nameserverArray,
      );
      ...
      console.error(
        "[Set Custom Nameservers API] Error setting custom nameservers:",
        error.message,
      );
